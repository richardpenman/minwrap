...
def extend(examples):
    """Attempt abstacting these examples from known vertical data.
    ...
    if scores:
        ...
        if num_perfect_labels == 1:
            ...
            common.logger.info('Multiple labels match all examples: {}'.format([label for label in scores if scores[label] == len(examples)]))
